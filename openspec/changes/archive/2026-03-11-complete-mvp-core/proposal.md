...
当前项目已经覆盖了 MVP 的基础浏览能力，但与 PRD 对照后，仍缺少两块核心价值：一是首页内的引导与状态体验还不完整，二是 `copenclaw` 手动更新链路还停留在原型阶段，尚不能稳定地产出符合约束的数据。现在补齐这两部分，可以让项目从“可演示”进入“可日常使用和维护”的状态。
...
- 完成 `copenclaw` 的主要数据更新流程：支持 RSS/HTML 数据源抓取、URL 规范化去重、基础标签/摘要补全、Schema 校验、按阈值分片输出和 `manifest.json` 生成。
...
- 数据更新工具：`tools/copenclaw/src/cli.ts`
