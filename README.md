...
- 支持主页与本地编辑器 UI 的中英双语切换（`zh-CN` / `en`）
- 语言默认跟随浏览器，也支持手动切换并记住本地偏好
- 国际化当前只覆盖界面文案，不翻译条目本身的标题、摘要、标签等内容
...
- 当前编辑器前端已迁移为 `Lit` 本地客户端，由 `tools/navhoard-cli/src/editor.ts` 提供页面与 API 服务
